...
class YunnanBaiyaoTrendPredictor:
    ...
    def load_data(self):
        ...
        print("CSV文件列名（原始）:", self.df.columns.tolist())
        
        # 如果列名是乱码，尝试根据位置推断列名
        if len(self.df.columns) >= 7:
            # 检查第一列是否是日期格式
            first_col = self.df.columns[0]
            sample_value = str(self.df.iloc[0, 0])
            if '/' in sample_value or '-' in sample_value:
                # 根据列数推断列名
                if len(self.df.columns) == 8:
                    # 8列格式：日期、收盘、开盘、高、低、交易量、成交额、涨跌幅
                    expected_columns = ['日期', '收盘', '开盘', '高', '低', '交易量', '成交额', '涨跌幅']
                elif len(self.df.columns) == 7:
                    # 7列格式：日期、收盘、开盘、高、低、交易量、涨跌幅
                    expected_columns = ['日期', '收盘', '开盘', '高', '低', '交易量', '涨跌幅']
                else:
                    expected_columns = None
                
                # 检查是否有中文列名，如果没有则使用位置推断
                if expected_columns:
                    has_chinese = any(any('\u4e00' <= char <= '\u9fff' for char in str(col)) for col in self.df.columns)
                    if not has_chinese and len(self.df.columns) == len(expected_columns):
                        print(f"检测到列名为乱码，根据位置自动推断列名（{len(expected_columns)}列）")
                        self.df.columns = expected_columns
        
        # 删除包含"数据来源"的行
        date_col = None
        for col in self.df.columns:
            if '日期' in str(col) or 'date' in str(col).lower() or col == 0:
                date_col = col
                break
        
        if date_col:
            self.df = self.df[~self.df[date_col].astype(str).str.contains('数据来源', na=False)]
    ...
    def plot_results(self, future_dates, predicted_prices):
        ...
        try:
            ...
            save_path = os.path.join(save_dir, 'yunnanbaiyao_trend_prediction_june_allhistory.png')
            ...
            ax1.set_title('云南白药股票价格（全部历史）', pad=15, fontproperties='SimHei')
            ...
            ax2.set_title('云南白药股票价格（2026年6月预测）', pad=15, fontproperties='SimHei')
...
def main():
    ...
    csv_path = os.path.join(script_dir, '000538.csv')
    predictor = YunnanBaiyaoTrendPredictor(csv_path)
    predictor.load_data()
    ...
    print('预测图表已保存为 yunnanbaiyao_trend_prediction_june_allhistory.png')
